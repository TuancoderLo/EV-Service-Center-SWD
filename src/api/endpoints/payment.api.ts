...
export const paymentApi = {
   ...
   */
  createPaymentIntent: async (data: CreatePaymentRequest): Promise<PaymentIntent> => {
    try {
      if (USE_MOCK_DATA) {
      ...
      const response = await httpClient.post<PaymentIntent>('/payments/create-intent', data);
      return response.data;
    } catch (error) {
      throw normalizeHttpError(error);
    }
   ...
   * Process payment
   */
  processPayment: async (paymentIntentId: string, paymentMethodId: string): Promise<Payment> => {
    try {
      if (USE_MOCK_DATA) {
  ...
  refundPayment: async (paymentId: string, amount?: number): Promise<Payment> => {
    try {
      ...
      const response = await httpClient.post<Payment>(`/payments/${paymentId}/refund`, {
        amount,
      });
      return response.data;
    } catch (error) {
