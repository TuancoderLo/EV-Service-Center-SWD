  ...
  getScheduleByTechnician: async (
    ...
    try {
      if (USE_MOCK_DATA) {
        ...
        let filtered = MOCK_SCHEDULES.filter(
      ...
      const response = await httpClient.get<Schedule[]>(
  ...
  getAvailableSlots: async (
    ...
    try {
      if (USE_MOCK_DATA) {
        ...
        MOCK_SCHEDULES.filter((s) => s.date === date && s.isAvailable).forEach(
          (schedule) => {
            schedule.timeSlots
    ...
    try {
      if (USE_MOCK_DATA) {
        ...
        const updatedTimeSlots = schedule.timeSlots.map((slot) =>
          slot.id === slotId ? { ...slot, isBooked: true, bookingId } : slot
        );
      ...
      const response = await httpClient.patch<Schedule>(
        `/schedules/${scheduleId}/book-slot`,
        {
          slotId,
          bookingId,
        }
